...
export default function ShareButtons({ url, title, compact = false }: Props) {
  ...
  const iconBase = `${iconSize} rounded-full flex items-center justify-center transition-transform group-hover:scale-110 active:scale-95`;
  const svgSize = compact ? "w-3.5 h-3.5" : "w-5 h-5";

  return (
    <div className={compact ? "flex items-center gap-2" : "flex flex-wrap gap-5 justify-center"}>
      {/* X */}
      <button onClick={shareToX} className={btnBase} aria-label="Xでシェア">
        <span className={`${iconBase} bg-black text-white`}>
          <svg viewBox="0 0 24 24" fill="currentColor" className={svgSize}>
        ...
        {!compact && <span className="text-xs text-muted">X</span>}
      </button>

      {/* LINE */}
      <button onClick={shareToLine} className={btnBase} aria-label="LINEでシェア">
        <span className={`${iconBase} bg-[#06C755] text-white`}>
          <svg viewBox="0 0 24 24" fill="currentColor" className={compact ? "w-4 h-4" : "w-6 h-6"}>
        ...
        {!compact && <span className="text-xs text-muted">LINE</span>}
      </button>
      ...
      <button onClick={shareToFacebook} className={btnBase} aria-label="Facebookでシェア">
        ...
        {!compact && <span className="text-xs text-muted">Facebook</span>}
      </button>
      ...
      {canShare && (
        <button onClick={shareToInstagram} className={btnBase} aria-label="Instagramでシェア">
          ...
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={svgSize}>
              <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
              <circle cx="12" cy="12" r="4" />
              <circle cx="17.5" cy="6.5" r="0.5" fill="currentColor" stroke="none" />
            </svg>
          </span>
          {!compact && <span className="text-xs text-muted">Instagram</span>}
        </button>
      )}
      ...
      <button onClick={copyUrl} className={btnBase} aria-label="URLをコピー">
        <span className={`${iconBase} bg-gray-200 text-gray-600`}>
          {copied ? (
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={svgSize}>
